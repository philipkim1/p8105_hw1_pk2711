...
In this dataset, it includes important information of the penguins which include the `species` of the penguin, the `island` they are from...There are `r nrow(penguins)` penguins represented in this dataset with `r ncol(penguins)` different variables describing each penguin listed. The mean flipper length is 200.92mm.
