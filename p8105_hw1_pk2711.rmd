...
In this dataset, it includes important information of the penguins which include the `species` of the penguin (3 in total), the `island` they are from, the bill dimensions such as `bill_length_mm` and `bill_depth_mm`, body mass in grams, and the `sex` of the penguins. There are `r nrow(penguins)` penguins represented in this dataset with `r ncol(penguins)` different variables describing each penguin listed. Accordingly, the mean flipper length of all the penguins listed is 200.92mm.
